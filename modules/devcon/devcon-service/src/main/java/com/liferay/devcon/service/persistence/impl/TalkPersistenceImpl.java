...
import com.liferay.devcon.model.impl.TalkImpl;
import com.liferay.devcon.model.impl.TalkModelImpl;
import com.liferay.devcon.service.persistence.TalkPersistence;
import com.liferay.devcon.service.persistence.impl.constants.PersistenceConstants;
 ...
 * @generated
 */
@Component(service = TalkPersistence.class)
public class TalkPersistenceImpl
	extends BasePersistenceImpl<Talk> implements TalkPersistence {

	/*
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never modify or reference this class directly. Always use <code>TalkUtil</code> to access the talk persistence. Modify <code>service.xml</code> and rerun ServiceBuilder to regenerate this class.
	 */
	public static final String FINDER_CLASS_NAME_ENTITY =
		TalkImpl.class.getName();
	...
	public List<Talk> findByUuid(
		...
		if (useFinderCache) {
			list = (List<Talk>)finderCache.getResult(
				finderPath, finderArgs, this);

			if ((list != null) && !list.isEmpty()) {
				for (Talk talk : list) {
					if (!uuid.equals(talk.getUuid())) {
						list = null;
		...
		if (list == null) {
			...
			sb.append(_SQL_SELECT_TALK_WHERE);
			...
			if (orderByComparator != null) {
				...
				sb.append(TalkModelImpl.ORDER_BY_JPQL);
			}
	...
	public Talk findByUuid_First(
		...
		throw new NoSuchTalkException(sb.toString());
	}
	...
	public Talk[] findByUuid_PrevAndNext(
		...
		try {
			...
			array[0] = getByUuid_PrevAndNext(
				session, talk, uuid, orderByComparator, true);

			array[1] = talk;

			array[2] = getByUuid_PrevAndNext(
				session, talk, uuid, orderByComparator, false);
	...
	protected Talk getByUuid_PrevAndNext(
		Session session, Talk talk, String uuid,
		...
		if (orderByComparator != null) {
			for (Object orderByConditionValue :
					orderByComparator.getOrderByConditionValues(talk)) {

				queryPos.add(orderByConditionValue);
			}
		}

		List<Talk> list = query.list();
	...
	public int countByUuid(String uuid) {
		...
		if (count == null) {
			...
			sb.append(_SQL_COUNT_TALK_WHERE);
	...
	private static final String _FINDER_COLUMN_UUID_UUID_3 =
		"(talk.uuid IS NULL OR talk.uuid = '')";
	...
	private static final String _FINDER_COLUMN_UUID_G_UUID_3 =
		"(talk.uuid IS NULL OR talk.uuid = '') AND ";
	...
	public Talk[] findByUuid_C_PrevAndNext(
		...
		try {
			...
			array[0] = getByUuid_C_PrevAndNext(
				session, talk, uuid, companyId, orderByComparator, true);

			array[1] = talk;

			array[2] = getByUuid_C_PrevAndNext(
				session, talk, uuid, companyId, orderByComparator, false);
	...
	protected Talk getByUuid_C_PrevAndNext(
		Session session, Talk talk, String uuid, long companyId,
		OrderByComparator<Talk> orderByComparator, boolean previous) {
	 ...
	 */
	@Override
	public List<Talk> findBystartDate(Date startDate) {
	 ...
	 */
	@Override
	public List<Talk> findBystartDate(Date startDate, int start, int end) {
	 ...
	 */
	@Override
	public List<Talk> findBystartDate(
	...
	public List<Talk> findBystartDate(
		...
		if ((start == QueryUtil.ALL_POS) && (end == QueryUtil.ALL_POS) &&
			...
			if (useFinderCache) {
				...
				finderArgs = new Object[] {_getTime(startDate)};
			}
		}
		else if (useFinderCache) {
			finderPath = _finderPathWithPaginationFindBystartDate;
			finderArgs = new Object[] {
				_getTime(startDate), start, end, orderByComparator
			};
		}
		...
		if (useFinderCache) {
			...
			if ((list != null) && !list.isEmpty()) {
				for (Talk talk : list) {
					if (!Objects.equals(startDate, talk.getStartDate())) {
						list = null;
	...
	public Talk findBystartDate_First(
		...
		Talk talk = fetchBystartDate_First(startDate, orderByComparator);
	...
	public Talk fetchBystartDate_First(
		...
		List<Talk> list = findBystartDate(startDate, 0, 1, orderByComparator);
	...
	public Talk findBystartDate_Last(
		...
		Talk talk = fetchBystartDate_Last(startDate, orderByComparator);
	...
	public Talk fetchBystartDate_Last(
		...
		List<Talk> list = findBystartDate(
	...
	public Talk[] findBystartDate_PrevAndNext(
		...
		try {
			...
			array[0] = getBystartDate_PrevAndNext(
				session, talk, startDate, orderByComparator, true);

			array[1] = talk;

			array[2] = getBystartDate_PrevAndNext(
				session, talk, startDate, orderByComparator, false);
	...
	protected Talk getBystartDate_PrevAndNext(
		Session session, Talk talk, Date startDate,
		OrderByComparator<Talk> orderByComparator, boolean previous) {
	...
	public int countBystartDate(Date startDate) {
		...
		Object[] finderArgs = new Object[] {_getTime(startDate)};
	...
	public TalkPersistenceImpl() {
		...
		setModelImplClass(TalkImpl.class);
	...
	public void cacheResult(Talk talk) {
		entityCache.putResult(TalkImpl.class, talk.getPrimaryKey(), talk);
	...
	public void cacheResult(List<Talk> talks) {
		for (Talk talk : talks) {
			if (entityCache.getResult(TalkImpl.class, talk.getPrimaryKey()) ==
					null) {
	...
	public void clearCache(Talk talk) {
		entityCache.removeResult(TalkImpl.class, talk);
	}
	...
	public void clearCache(Set<Serializable> primaryKeys) {
		...
		for (Serializable primaryKey : primaryKeys) {
			entityCache.removeResult(TalkImpl.class, primaryKey);
		}
	}
	...
	protected void cacheUniqueFindersCache(TalkModelImpl talkModelImpl) {
		...
		finderCache.putResult(
			_finderPathFetchByUUID_G, args, talkModelImpl, false);
	}
	...
	public Talk create(long talkId) {
		...
		talk.setCompanyId(CompanyThreadLocal.getCompanyId());
	...
	public Talk remove(Serializable primaryKey) throws NoSuchTalkException {
		...
		catch (NoSuchTalkException noSuchEntityException) {
	...
	protected Talk removeImpl(Talk talk) {
		...
		try {
			...
			if (!session.contains(talk)) {
				talk = (Talk)session.get(
					TalkImpl.class, talk.getPrimaryKeyObj());
			}
	...
	public Talk updateImpl(Talk talk) {
		...
		if (!(talk instanceof TalkModelImpl)) {
			InvocationHandler invocationHandler = null;

			if (ProxyUtil.isProxyClass(talk.getClass())) {
				invocationHandler = ProxyUtil.getInvocationHandler(talk);

				throw new IllegalArgumentException(
					"Implement ModelWrapper in talk proxy " +
						invocationHandler.getClass());
			}

			throw new IllegalArgumentException(
				"Implement ModelWrapper in custom Talk implementation " +
					talk.getClass());
		}
		...
		entityCache.putResult(TalkImpl.class, talkModelImpl, false, true);
	...
	/**
	 * Returns the talk with the primary key or throws a <code>com.liferay.portal.kernel.exception.NoSuchModelException</code> if it could not be found.
	 *
	 * @param primaryKey the primary key of the talk
	 * @return the talk
	 * @throws NoSuchTalkException if a talk with the primary key could not be found
	...
	public int countAll() {
		...
		if (count == null) {
			...
			try {
				...
				Query query = session.createQuery(_SQL_COUNT_TALK);
	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			ArgumentsResolver.class, new TalkModelArgumentsResolver(),
			MapUtil.singletonDictionary(
				"model.class.name", Talk.class.getName()));
	...
	private static final String _SQL_SELECT_TALK_WHERE =
		"SELECT talk FROM Talk talk WHERE ";

	private static final String _SQL_COUNT_TALK =
		"SELECT COUNT(talk) FROM Talk talk";

	private static final String _SQL_COUNT_TALK_WHERE =
		"SELECT COUNT(talk) FROM Talk talk WHERE ";

	private static final String _ORDER_BY_ENTITY_ALIAS = "talk.";

	private static final String _NO_SUCH_ENTITY_WITH_PRIMARY_KEY =
		"No Talk exists with the primary key ";

	private static final String _NO_SUCH_ENTITY_WITH_KEY =
		"No Talk exists with the key {";

	private static final Log _log = LogFactoryUtil.getLog(
		TalkPersistenceImpl.class);
	...
	static {
		try {
			Class.forName(PersistenceConstants.class.getName());
		}
	...
	private static class TalkModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			TalkModelImpl talkModelImpl = (TalkModelImpl)baseModel;

			long columnBitmask = talkModelImpl.getColumnBitmask();

			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(talkModelImpl, columnNames, original);
			}
			...
			if (finderPathColumnBitmask == null) {
				...
				for (String columnName : columnNames) {
					finderPathColumnBitmask |= talkModelImpl.getColumnBitmask(
						columnName);
				}
		...
		private Object[] _getValue(
			TalkModelImpl talkModelImpl, String[] columnNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = talkModelImpl.getColumnOriginalValue(
						columnName);
				}
				else {
					arguments[i] = talkModelImpl.getColumnValue(columnName);
				}
			}
